...
#include <vector>
#include <map>
// naive approach, can be optimized
using std::map;
using std::string;
using std::vector;
const char* INFILENAME = "rosalind_ba1j.txt";
const char* OUTFILENAME = "out.txt";

string revcmp(string const& s)
{
    static const map<char, char> Compl = {
        {'A', 'T'},
        {'T', 'A'},
        {'C', 'G'},
        {'G', 'C'}
...
void kmerProcessor::processAllKmersRec(string const& prefix, int const k, int const d)
{
    ...
    {   
        ...
        for (auto const& item: kmers)
        {
            ...
            if (hammingDistanceLEd(rev, item.first, d)) // if revcmp dist <= d
            {
                count += item.second;
            }
        }
